...
public class bookd2 {
  ...
  public static void main(String[] args) {
    ...
    HashMap<Integer, String> solution = new HashMap<Integer, String>();
    ...
    int[] input = new int[inputA.size()];
    for (int i = 0; i < input.length; i++) {
    	input[i] = inputA.get(i);
    }
    int[] inputOriginal = input.clone();
    Arrays.sort(input);
    int current = 0;
    int currentSum = 0;
    for (int i : input) {
    	...
    	solution.put(i, (currentSum - i) + " " + current);
    }
    for (int i : inputOriginal) {
    	sw.printLine(solution.get(i));
    }
    sw.close();
  }
